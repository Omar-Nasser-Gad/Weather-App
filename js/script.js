const apiKey=`ea07b53c7d544583aa5162931240607`
...
searchLocation.addEventListener("change", function () {
  ...
  async function getWeather(country) {
    ...
    document.querySelector(".weatherCards").innerHTML = `<div class="lds-ring"><div></div><div></div><div></div><div></div></div>`
      let response = await fetch(`${BaseURL}?key=${apiKey}&q=${country}&days=3`)
    ...
    for (i = 0; i < dataArray.length; i++) {
      ...
      weatherBox += `
                  <div class="today flex-grow-1 m-2">
                      <div class="innerCard bg-danger rounded-3 py-2 px-4">
                          ...
                          <p class="text-center fw-semibold text-dark m-0 p-0">${dataArray[i].day.condition.text}</p>
